...
const PatientProfile = () => {
  ...
  // ✅ FIXED: Get staff name with proper fallback
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // Build recent activities with CORRECT staff mappings
  ...
  const buildRecentActivities = (patientData, vitalsData, imagingData, notesData, prescriptionsData, labData) => {
    ...
    if (vitalsData && vitalsData.length > 0) {
      vitalsData.forEach(v => {
        activities.push({
          ...
          staff: v.nurse,
          staffName: getStaffName(v.nurse),
          icon: '❤️',
          color: '#ef4444',
          details: v
        });
    ...
    if (notesData && notesData.length > 0) {
      notesData.forEach(n => {
        activities.push({
          ...
          staff: n.author,
          staffName: getStaffName(n.author),
          icon: '📝',
          color: '#3b82f6',
          details: n
        });
    ...
    // ✅ Prescriptions - Show dispensedBy if dispensed, otherwise prescribedBy
    if (prescriptionsData && prescriptionsData.length > 0) {
      prescriptionsData.forEach(p => {
        ...
        activities.push({
          ...
          // ✅ Show dispensedBy if dispensed, otherwise prescribedBy
    ...
    // ✅ Lab Orders - Show performedBy (who did the test), fallback to orderedBy
    if (labData && labData.length > 0) {
      labData.forEach(l => {
        // Show performedBy if available (Lab Technician/Scientist), otherwise orderedBy (Doctor)
    ...
    // ✅ Imaging Orders - Show radiologist if completed, otherwise orderingStaff
    ...
    <div className="patient-profile-container">
              ...
              notes.map(n => (
                <div key={n.id} className={`note-card type-${n.type.replace(/ /g, '')}`}>
                  <div className="note-header">
                    ...
                    <span><strong>{n.type}</strong> by {getStaffName(n.author)}</span>
            ...
            <table>
              ...
              <tbody>
                {prescriptions.length > 0 ? (
                  ...
                  <tr><td colSpan="6" className="text-center">No prescriptions found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showNoteModal && (
        <div className="modal-overlay profile-modal" onClick={() => setShowNoteModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h3>{editingNote ? 'Edit Clinical Note' : 'Add Clinical Note'}</h3><button className="modal-close" onClick={() => setShowNoteModal(false)}>×</button></div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handlePrescriptionSubmit}>
              <div className="modal-body">
                ...
                <div className="form-row"><div className="form-group"><label>Dosage *</label><input type="text" required value={prescriptionForm.dosage} onChange={e => setPrescriptionForm({...prescriptionForm, dosage: e.target.value})} placeholder="e.g. 1 tablet" /></div><div className="form-group"><label>Frequency *</label><input type="text" required value={prescriptionForm.frequency} onChange={e => setPrescriptionForm({...prescriptionForm, frequency: e.target.value})} placeholder="e.g. Twice daily" /></div></div>
                <div className="form-row"><div className="form-group"><label>Duration</label><input type="text" value={prescriptionForm.duration} onChange={e => setPrescriptionForm({...prescriptionForm, duration: e.target.value})} placeholder="e.g. 7 days" /></div><div className="form-group"><label>Instructions</label><input type="text" value={prescriptionForm.instructions} onChange={e => setPrescriptionForm({...prescriptionForm, instructions: e.target.value})} placeholder="e.g. Take with food" /></div></div>
              </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleLabOrderSubmit}>
              <div className="modal-body">
                ...
                <div className="form-row"><div className="form-group"><label>Test Type *</label><select value={labOrderForm.testType} onChange={e => setLabOrderForm({...labOrderForm, testType: e.target.value})}><option>Haematology</option><option>Biochemistry</option><option>Microbiology</option><option>Radiology</option></select></div><div className="form-group"><label>Priority *</label><select value={labOrderForm.priority} onChange={e => setLabOrderForm({...labOrderForm, priority: e.target.value})}><option>Routine</option><option>Urgent</option><option>Emergency</option></select></div></div>
      ...
      {showOrderModal && selectedOrder && (
        <div className="modal-overlay" onClick={() => setShowOrderModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '700px' }}>
            <div className="modal-header"><h3>📷 Imaging Order Details</h3><button className="modal-close" onClick={() => setShowOrderModal(false)}>×</button></div>
            <div className="modal-body">
      ...
      {showImagingModal && (
        <div className="modal-overlay" onClick={() => setShowImagingModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '650px' }}>
            <div className="modal-header"><h3>📷 New Imaging/X-Ray Order</h3><button className="modal-close" onClick={() => setShowImagingModal(false)}>×</button></div>
